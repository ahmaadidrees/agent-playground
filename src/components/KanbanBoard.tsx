...
import { Plus, ChevronRight, ChevronLeft, MoreHorizontal, CheckCircle2, Clock, PlayCircle, Inbox, Trash2 } from 'lucide-react'
...
const statusOrder: TaskStatus[] = ['proposed', 'backlog', 'in_progress', 'done']
        ...
        <form onSubmit={handleAddTask} className="flex items-center gap-2">
          <div className="relative">
            <input
              ...
              className="pl-4 pr-10 py-2.5 bg-white border border-amber-900/10 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500/50 transition-all w-64 shadow-sm"
        ...
        {statusOrder.map((status) => {
          ...
          return (
            <div key={status} className="flex flex-col gap-3 min-h-0">
              ...
              <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar flex flex-col gap-3 min-h-0 pb-4">
                <AnimatePresence mode="popLayout">
                  {columnTasks.length === 0 ? (
                    <motion.div
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className="h-24 rounded-2xl border-2 border-dashed border-amber-900/5 flex items-center justify-center text-[11px] text-amber-900/20 font-medium text-center px-4"
                    >
                      Drop tasks here
                    </motion.div>
                  ) : (
